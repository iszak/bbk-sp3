...
class PublicVirtualMachine(stack:Vector[Int]) extends VirtualMachine {
  ...
  override def execute(bc: Vector[ByteCode]): VirtualMachine = {
    val initialValue = Tuple2[Vector[ByteCode], VirtualMachine](bc, this)

    bc.foldLeft(initialValue)((accumulator, _) => {
      val bc = accumulator._1
      val vm = accumulator._2
  ...
  override def executeOne(bc: Vector[ByteCode]): (Vector[ByteCode], VirtualMachine) = {
    val inst = bc(0)
    val newVm = inst.execute(this)
    (bc.takeRight(bc.length - 1), newVm)
  }
  ...
  /**
    ...
    */
  override def push(value: Int): VirtualMachine = new PublicVirtualMachine(stack :+ value)
